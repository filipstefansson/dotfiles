...
- **skills.sh** — Reinstalls agent skills via the `skills` CLI (`npx skills add <repo>`), installed globally and symlinked into Codex + Codex. Runs after npm.sh (needs Node)
...
- **zoxide** replaces `cd` — initialized in `.zshrc` with a Codex guard (`Codex` env var)
